# GaAs-only Keldysh + avalanche-ionization workflow.
# Standalone full workflow ported from Keldsyh_II_NaCl.py.

# %%
# Cell 0
"""
Reconciled Keldysh + avalanche ionization model for GaAs.
...
Default workflow
----------------
The default ``--mode all`` execution produces the GaAs 0.8-um reference case
specified in ``GaAs_Keldysh_Parameter.docx``. The full NaCl/BaF2 plot, ODE,
multi-case, 3D-surface, CSV, JSON-manifest, and saved-variable framework is
retained. Only the single documented case is enabled by default.
"""
...
# CODEX MODIFICATION START: constants from GaAs verified reference / CODATA
C0 = 299792458.0
EPS0 = 8.8541878188e-12
E_CHARGE = 1.602176634e-19
HBAR = 1.054571817e-34
ME0 = 9.1093837139e-31
# CODEX MODIFICATION END: constants from GaAs verified reference / CODATA
...
# Source: GaAs_Keldysh_Parameter.docx (room-temperature 800-nm reference).
# The reduced mass is calculated from the Gamma-point electron and heavy-hole
# masses. kappa is recorded in the input and exports but is deliberately not
# used by this inherited lossless zero-D Keldysh/avalanche calculation.
GAAS_INPUT: Dict[str, Any] = {
    "reference_file": "../input_parameters/GaAs_Keldysh_Parameter.docx",
    "material": {
        "name": "GaAs",
        "short": "GaAs",
        "mat_flag": 1,
        "bandgap_ev": 1.424,
        "gamma_transition": {
            "electron_mass_over_me": 0.067,
            "heavy_hole_mass_over_me": 0.50,
            "note": "Gamma-point direct electron-heavy-hole transition.",
            "reduced_mass_definition": "Gamma electron-heavy-hole reduced mass",
        },
        "parameter_status": (
            "800 nm is above the 300 K direct gap. The input index is "
            "n+i*kappa = 3.682+0.069i, but this zero-D Keldysh/avalanche "
            "result uses only Re(n); it is diagnostic only and omits direct "
            "one-photon interband absorption, complex-index propagation, and loss."
        ),
        "n2_m2_per_w": np.nan,
        "optical_index": {
            "wavelength_um": 0.8,
            ...
            "note": (
                "Room-temperature bulk GaAs n+i*kappa. The calculation uses "
                "n_real only; kappa is retained for reporting and future models."
            ),
        },
        "model_status": {"diagnostic_only": True},
        "known_limitations": (
            "At 800 nm, direct one-photon interband absorption and complex-index "
            "propagation are omitted; density output is diagnostic only.",
            "The legacy Drude/avalanche term reuses the transition reduced mass; "
            "a future model should use a conductivity mass separately.",
    ...
    "cases": [
        {
            ...
            "input_note": (
                ...
                "from GaAs_Keldysh_Parameter.docx. Reflection is neglected, so "
                "the surface fluence is used directly as the zero-dimensional "
                "effective fluence."
            ),
        },
    ],
...
# CODEX MODIFICATION START: GaAs-only material parameters
def material_flag1(
    mat_flag: int,
    wavelength_um: float,
) -> Tuple[float, float, float, float, float]:
    """
    Return the documented 800-nm GaAs material parameters.

    Parameters
    ----------
    mat_flag:
        Material selector. This standalone script uses only 1 for GaAs.
    wavelength_um:
    ...
    n2:
        ...
        GaAs Keldysh-parameter reference document.
    Eg_J:
    ...
    if mat_flag != 1:
        raise ValueError("mat_flag must be 1 for GaAs.")

    material_input = GAAS_INPUT["material"]
    bandgap_ev = float(material_input["bandgap_ev"])
    transition = material_input["gamma_transition"]
    electron_mass = float(transition["electron_mass_over_me"])
    heavy_hole_mass = float(transition["heavy_hole_mass_over_me"])
    mred_over_me = electron_mass * heavy_hole_mass / (electron_mass + heavy_hole_mass)
    ...
    if not valid_min <= lam <= valid_max:
        raise ValueError(
            f"GaAs n+i*kappa is specified only for {valid_min}-{valid_max} um; "
            f"received {lam} um."
        )
    # kappa is intentionally not used. This preserves the same real-index,
    # zero-D Keldysh/avalanche logic as the NaCl and BaF2 scripts.
...
# CODEX MODIFICATION END: GaAs-only material parameters


# CODEX MODIFICATION START: GaAs-only case definitions
def get_cases() -> List[CaseDict]:
    """
    Return the currently enabled GaAs cases.

    Returns
    -------
    list of dict
        GaAs material/laser case dictionaries. The list-based design retains
        the complete multi-case framework even though one documented case is
        enabled by default.
    """

    cases: List[CaseDict] = []
    for case_input in GAAS_INPUT["cases"]:
        ...
        cases.append(case)
    return cases
# CODEX MODIFICATION END: GaAs-only case definitions
...
def display_summary_table(rows: Sequence[Dict[str, Any]]) -> None:
    ...
    # CODEX MODIFICATION START: display optional GaAs reference fields cleanly
    ...
    # CODEX MODIFICATION END: display optional GaAs reference fields cleanly
...
# ============================================================
# GaAs-reference Keldysh parameter axis
# ============================================================

# CODEX MODIFICATION START: GaAs-reference Keldysh parameter axis
def gamma_gaas_reference_from_intensity_wcm2(
    I_wcm2: ArrayLike,
    wavelength_um: float,
) -> ArrayLike:
    """
    Evaluate the Keldysh parameter using GaAs as the reference material.
    ...
    if np.any(valid):
        ...
        n_gaas, _n2, Eg_gaas_J, mred_gaas, _trans = material_flag1(1, wavelength_um)
        omega = 2.0 * np.pi * C0 / (wavelength_um * 1.0e-6)
        with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
            gamma_valid = (omega / E_CHARGE) * np.sqrt(
                (mred_gaas * C0 * n_gaas * EPS0 * Eg_gaas_J)
                / I_wm2
            )
...
def intensity_wcm2_from_gamma_gaas_reference(
    gamma: ArrayLike,
    wavelength_um: float,
) -> ArrayLike:
    """
    Convert a GaAs-reference Keldysh parameter to intensity in W/cm^2.
    """
...
def add_gaas_gamma_top_axis(
    ax: plt.Axes,
    wavelength_um: float,
    gamma_ticks: Tuple[float, ...],
) -> None:
    """
    Add a GaAs-reference Keldysh-parameter axis above an intensity axis.
    """
    ...
    tick_positions = np.asarray(
        intensity_wcm2_from_gamma_gaas_reference(
    ...
    ax_top.set_xlabel(r"Keldysh parameter $\gamma$ (GaAs reference)")
    ax_top.tick_params(axis="x", which="both", direction="in")
# CODEX MODIFICATION END: GaAs-reference Keldysh parameter axis
...
def plot_total_ionization_nir_lwir_comparison_with_gamma_axis(
    ...
    """
    Plot peak total ionization rate for the enabled GaAs wavelength regimes.

    Each panel includes a GaAs-reference Keldysh-parameter top axis and a
    ...
    if not enabled_regimes:
        raise ValueError("At least one GaAs NIR or LWIR case is required.")
    panel_labels = {"NIR": "(a) NIR", "LWIR": "(b) LWIR"}
    gamma_ticks_by_regime = {
        "NIR": (10.0, 3.0, 1.0, 0.3),
        "LWIR": (1.0, 0.3, 0.1),
    }
    material_order = {"GaAs": 0}
...
def plot_material_figure_4_style(
    ...
    save_or_show(
        fig,
        save_dir,
        "GaAs_fig4_NIR_scaling.png",
    )


# CODEX MODIFICATION START: GaAs-only plotting workflow
def plot_gaas_figures(
    cases: Sequence[CaseDict],
    n_intensity_points: int = 70,
    save_dir: Optional[Path] = None,
) -> None:
    """Generate all Figs. 1-4 for GaAs."""

    print("\n============================================================")
    print("Generating GaAs plots")
...
# CODEX MODIFICATION END: GaAs-only plotting workflow
...
def plot_total_ionization_3d_surface_grid(
    ...
    """Plot all GaAs total-ionization 3D surfaces in one figure."""
    ...
    fig.suptitle(
        rf"Total ionization surfaces: GaAs {enabled_regions}",
        fontsize=15,
    )
    save_or_show(
        fig=fig,
        save_dir=save_dir,
        filename=f"05_total_ionization_3d_GaAs_{'_'.join(str(case['region']) for case in cases)}.png",
...
def _case_export_metadata(case: CaseDict) -> Dict[str, Any]:
    ...
    metadata: Dict[str, Any] = {
        ...
        "reduced_mass_definition": str(
            GAAS_INPUT["material"]["gamma_transition"]["reduced_mass_definition"]
        ),
        "gamma_electron_mass_over_me": float(
            GAAS_INPUT["material"]["gamma_transition"]["electron_mass_over_me"]
        ),
        "gamma_heavy_hole_mass_over_me": float(
            GAAS_INPUT["material"]["gamma_transition"]["heavy_hole_mass_over_me"]
        ),
        "kappa_recorded_not_used": float(
            GAAS_INPUT["material"]["optical_index"]["kappa"]
        ),
        "transmission_factor": float(trans),
        "reference_file": str(GAAS_INPUT["reference_file"]),
        "diagnostic_only": bool(
            GAAS_INPUT["material"]["model_status"]["diagnostic_only"]
        ),
        "parameter_status": str(GAAS_INPUT["material"]["parameter_status"]),
    }
    return metadata
...
def save_gaas_plot_variables(
    ...
    """Save long-format GaAs plot data as CSV tables only."""
    ...
    print("\nSaving labeled GaAs plot variables ...")
    for case in cases:
        short = str(case["short"])
        ...
        component_manifest[short] = {
            "used_by": ["GaAs_fig2_NIR.png", "GaAs_fig4_NIR_scaling.png"],
            "arrays": {},
        }
        ...
        component_gamma = np.asarray(
            gamma_gaas_reference_from_intensity_wcm2(
                component["I_wcm2"],
                wavelength_um=float(case["wavelength_um"]),
            ),
            dtype=float,
        )
        component_gamma_name = f"component__{short}__gamma_gaas_reference"
        arrays[component_gamma_name] = component_gamma
        component_manifest[short]["arrays"]["gamma_gaas_reference"] = component_gamma_name
            ...
            component_rows.append(
                {
                    **metadata,
                    ...
                    "gamma_gaas_reference": float(component_gamma[index]),
        ...
        total_gamma_name = f"total_comparison__{short}__gamma_gaas_reference"
        arrays[total_gamma_name] = total_gamma
        total_manifest[short]["arrays"]["gamma_gaas_reference"] = total_gamma_name
            ...
            total_rows.append(
                {
                    **metadata,
                    ...
                    "point_index": int(index),
                    "I_wcm2": float(intensity),
                    "gamma_gaas_reference": float(total_gamma[index]),
    ...
    manifest = {
        "description": "Labeled variables for GaAs Keldysh and avalanche-ionization plots.",
        ...
        "known_limitations": list(GAAS_INPUT["material"]["known_limitations"]),
        "units": {
            "I_wcm2": "W/cm^2",
            "F0_jcm2": "J/cm^2",
            "beam_radius_a_um": "um",
            ...
            "ne_grid_cm3": "cm^-3",
            "gamma_gaas_reference": "dimensionless",
        },
        "files": {
            "component_scaling_csv": "gaas_component_scaling_long.csv",
            "total_comparison_csv": "gaas_total_comparison_long.csv",
            "surface3d_csv": "gaas_surface3d_long.csv",
        },
        "component_scaling": component_manifest,
        "total_comparison": total_manifest,
        "surface3d": surface_manifest,
    }
    _write_csv(output_dir / "gaas_component_scaling_long.csv", component_rows)
    _write_csv(output_dir / "gaas_total_comparison_long.csv", total_rows)
    _write_csv(output_dir / "gaas_surface3d_long.csv", surface_rows)
    print(f"Saved GaAs CSV data to {output_dir}")
...
def solve_and_display_summary(cases: Sequence[CaseDict]) -> None:
    """Solve the GaAs cases and display the summary table."""
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run the reconciled GaAs Keldysh + avalanche model and "
    ...
    parser.add_argument(
        ...
        default="all",
        help=(
            "all: all GaAs plots and table; summary: first/last graph sets "
            "and table; figures: GaAs Figs. 1-4; 3d: total-ionization "
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Save every currently enabled GaAs figure and labeled numerical variables "
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        choices=range(len(GAAS_INPUT["cases"])),
        metavar="INDEX",
        help=(
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    material_input = GAAS_INPUT["material"]
    optical_index = material_input["optical_index"]
    print(f"Running full GaAs workflow ({len(cases)} enabled case(s))")
    print("Source-paper convention: E = sqrt(I/(c n eps0)); gamma uses I, not 2I")
    print(
        "Optical index input: "
        f"n+i*kappa = {optical_index['n_real']}+{optical_index['kappa']}i; "
    ...
    for case in cases:
        I0_wcm2 = case_reference_peak_intensity_wcm2(case)
        gamma0 = gamma_gaas_reference_from_intensity_wcm2(
            I0_wcm2,
            wavelength_um=float(case["wavelength_um"]),
        )
    ...
    if args.save:
        ...
        plot_gaas_figures(cases, args.points, save_dir)
    # CODEX MODIFICATION START: CLI support for 3D surface plot
    elif args.mode == "3d":
