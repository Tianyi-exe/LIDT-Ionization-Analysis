# IRG207-only Keldysh + avalanche-ionization workflow.
# Standalone full workflow ported from Keldsyh_II_NaCl.py.

# %%
# Cell 0
"""
Reconciled Keldysh + avalanche ionization model for IRG207.
...
Default workflow
----------------
The default ``--mode all`` execution produces the IRG207 0.8-um reference
case specified in ``IRG207_Keldysh_Parameter.docx``. The full NaCl/BaF2
...
# CODEX MODIFICATION START: constants from IRG207 verified reference / CODATA
C0 = 299792458.0
EPS0 = 8.8541878188e-12
E_CHARGE = 1.602176634e-19
HBAR = 1.054571817e-34
ME0 = 9.1093837139e-31
# CODEX MODIFICATION END: constants from IRG207 verified reference / CODATA
...
# Source: IRG207_Keldysh_Parameter.docx.
...
# NaCl/BaF2 calculation logic and are diagnostic IRG207 inputs, not a
# complete Ti3+ electronic-structure model.
IRG207_INPUT: Dict[str, Any] = {
    "reference_file": "../input_parameters/IRG207_Keldysh_Parameter.docx",
    "material": {
        "name": 'IRG207 infrared glass (IRG-24 / IG4-equivalent proxy)',
        "short": 'IRG207',
        "mat_flag": 1,
        "bandgap_ev": 1.7,
        "reduced_mass": {
            ...
            "status": "Temporary project input; use 1.0 m0 until a verified value is supplied.",
        },
        "parameter_status": 'The 800-nm index uses an IRG-24/IG4-equivalent proxy and the 1.70 eV gap is provisional. Operation is near the short-wave absorption edge; this Re(n)-only result is diagnostic.',
        "n2_m2_per_w": np.nan,
        "optical_index": {
            "wavelength_um": 0.8,
            "n_real": 2.8028,
            ...
            "note": 'SCHOTT IRG-24 value used as the stated IRG207 proxy.',
        },
        "model_status": {"diagnostic_only": True},
        "known_limitations": (
            "IRG207 identity/batch equivalence to IRG-24/IG4 and the 1.70 eV optical gap require confirmation.",
            "Short-wave absorption, kappa, reflection, and propagation are omitted at 800 nm.",
            "The legacy Drude/avalanche term reuses the placeholder reduced mass; a conductivity mass is not separately supplied.",
            "Carrier depletion, saturation, recombination, heating, and propagation are not included.",
            "The inherited qfun termination should be benchmarked before nonmonotonic rate features are assigned physical meaning.",
        ),
    },
    "cases": [
        {
            "name": r"IRG207, 0.8 $\mu$m",
            "short": "IRG207_NIR",
            "material": 'IRG207 infrared glass (IRG-24 / IG4-equivalent proxy)',
            "region": "NIR",
            "mat_flag": 1,
            "wavelength_um": 0.8,
            "tau_fs": 100.0,
            "pulse_duration_status": "BaF2 800-nm template value; confirm experimentally.",
            "reference_I0_wcm2": None,
            "F0_jcm2": 0.0485,
            "beam_diameter_a_um": 162.1,
            "beam_diameter_b_um": 152.4,
            "beam_diameter_definition": "full 1/e^2 intensity diameter",
            "fluence_basis": "surface incident",
            "input_note": (
                "F, a, and b follow IRG207_Keldysh_Parameter.docx. "
                "The surface fluence is used directly because reflection is not calculated. "
                "The 100 fs pulse duration is inherited from the BaF2 800-nm template and must be confirmed."
            ),
        },
    ],
}

# CODEX MODIFICATION START: IRG207-only material parameters
def material_flag1(
    mat_flag: int,
    wavelength_um: float,
) -> Tuple[float, float, float, float, float]:
    """Return the documented 800-nm IRG207 material parameters."""

    lam = float(wavelength_um)
    if lam <= 0.0:
        raise ValueError("wavelength_um must be positive.")
    if mat_flag != 1:
        raise ValueError("mat_flag must be 1 for IRG207.")

    material_input = IRG207_INPUT["material"]
    ...
    if not valid_min <= lam <= valid_max:
        raise ValueError(
            f"IRG207 optical constants are specified only for "
            f"{valid_min}-{valid_max} um; received {lam} um."
        )
    ...
    mred_over_me = float(
        material_input["reduced_mass"]["electron_hole_reduced_effective_mass_over_me"]
    )
    mred = mred_over_me * ME0
...
# CODEX MODIFICATION END: IRG207-only material parameters


# CODEX MODIFICATION START: IRG207-only case definitions
def get_cases() -> List[CaseDict]:
    """
    Return the currently enabled IRG207 cases.

    Returns
    -------
    list of dict
        IRG207 material/laser case dictionaries. The list-based design
        retains the complete multi-case framework even though one documented
        case is enabled by default.
    """

    cases: List[CaseDict] = []
    for case_input in IRG207_INPUT["cases"]:
        ...
        cases.append(case)
    return cases
# CODEX MODIFICATION END: IRG207-only case definitions
...
def elliptical_gaussian_peak_fluence_jcm2(
    ...
    The measured full diameters are converted to radii before evaluating

        F = 2 E / (pi r_a r_b),

    where ``r_a`` and ``r_b`` are the corresponding 1/e^2 intensity radii. This is the on-axis
        ...
        rows.append(
            {
                ...
                "tau_fs": case["tau_fs"],
                "F0_Jcm2": case.get("F0_jcm2"),
                "beam_diameter_a_um": case.get("beam_diameter_a_um"),
                "beam_diameter_b_um": case.get("beam_diameter_b_um"),
...
def display_summary_table(rows: Sequence[Dict[str, Any]]) -> None:
    ...
    # CODEX MODIFICATION START: display optional IRG207 reference fields cleanly
    ...
    # CODEX MODIFICATION END: display optional IRG207 reference fields cleanly
...
# ============================================================
# IRG207-reference Keldysh parameter axis
# ============================================================

# CODEX MODIFICATION START: IRG207-reference Keldysh parameter axis
def gamma_irg207_reference_from_intensity_wcm2(
    I_wcm2: ArrayLike,
    wavelength_um: float,
) -> ArrayLike:
    """
    Evaluate the Keldysh parameter using IRG207 as the reference material.
    ...
    if np.any(valid):
        ...
        n_irg207, _n2, Eg_irg207_J, mred_irg207, _trans = material_flag1(1, wavelength_um)
        omega = 2.0 * np.pi * C0 / (wavelength_um * 1.0e-6)
        with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
            gamma_valid = (omega / E_CHARGE) * np.sqrt(
                (mred_irg207 * C0 * n_irg207 * EPS0 * Eg_irg207_J)
                / I_wm2
            )
...
def intensity_wcm2_from_gamma_irg207_reference(
    gamma: ArrayLike,
    wavelength_um: float,
) -> ArrayLike:
    """
    Convert a IRG207-reference Keldysh parameter to intensity in W/cm^2.
    """
...
def add_irg207_gamma_top_axis(
    ax: plt.Axes,
    wavelength_um: float,
    gamma_ticks: Tuple[float, ...],
) -> None:
    """
    Add a IRG207-reference Keldysh-parameter axis above an intensity axis.
    """
    ...
    tick_positions = np.asarray(
        intensity_wcm2_from_gamma_irg207_reference(
    ...
    ax_top.set_xlabel(r"Keldysh parameter $\gamma$ (IRG207 reference)")
    ax_top.tick_params(axis="x", which="both", direction="in")
# CODEX MODIFICATION END: IRG207-reference Keldysh parameter axis
...
def plot_total_ionization_nir_lwir_comparison_with_gamma_axis(
    ...
    """
    Plot peak total ionization rate for the enabled IRG207 wavelength regimes.

    Each panel includes a IRG207-reference Keldysh-parameter top axis and a
    ...
    if not enabled_regimes:
        raise ValueError("At least one IRG207 NIR or LWIR case is required.")
    panel_labels = {"NIR": "(a) NIR", "LWIR": "(b) LWIR"}
    gamma_ticks_by_regime = {
        "NIR": (10.0, 3.0, 1.0, 0.3),
        "LWIR": (1.0, 0.3, 0.1),
    }
    material_order = {"IRG207": 0}
...
def plot_case_figure_1_style(
    ...
    save_or_show(
        fig,
        save_dir,
        f"IRG207_fig1_{case['region']}.png",
    )
...
def plot_case_figure_2_style(
    ...
    save_or_show(
        fig,
        save_dir,
        f"IRG207_fig2_{case['region']}.png",
    )
...
def plot_case_figure_3_style(
    ...
    save_or_show(
        fig,
        save_dir,
        f"IRG207_fig3_{case['region']}.png",
    )
...
def plot_material_figure_4_style(
    ...
    save_or_show(
        fig,
        save_dir,
        "IRG207_fig4_NIR_scaling.png",
    )


# CODEX MODIFICATION START: IRG207-only plotting workflow
def plot_irg207_figures(
    cases: Sequence[CaseDict],
    n_intensity_points: int = 70,
    save_dir: Optional[Path] = None,
) -> None:
    """Generate all Figs. 1-4 for IRG207."""

    print("\n============================================================")
    print("Generating IRG207 plots")
...
# CODEX MODIFICATION END: IRG207-only plotting workflow
...
def plot_total_ionization_3d_surface_grid(
    ...
    """Plot all IRG207 total-ionization 3D surfaces in one figure."""
    ...
    fig.suptitle(
        rf"Total ionization surfaces: IRG207 {enabled_regions}",
        fontsize=15,
    )
    save_or_show(
        fig=fig,
        save_dir=save_dir,
        filename=f"05_total_ionization_3d_IRG207_{'_'.join(str(case['region']) for case in cases)}.png",
...
def _case_export_metadata(case: CaseDict) -> Dict[str, Any]:
    ...
    metadata: Dict[str, Any] = {
        ...
        "beam_diameter_a_um": float(case["beam_diameter_a_um"]),
        "beam_diameter_b_um": float(case["beam_diameter_b_um"]),
        "beam_diameter_definition": str(case["beam_diameter_definition"]),
        ...
        "reduced_mass_definition": str(
            IRG207_INPUT["material"]["reduced_mass"]["definition"]
        ),
        "kappa_recorded_not_used": (
            float(IRG207_INPUT["material"]["optical_index"]["kappa"])
            if np.isfinite(float(IRG207_INPUT["material"]["optical_index"]["kappa"]))
            else None
        ),
        "transmission_factor": float(trans),
        "reference_file": str(IRG207_INPUT["reference_file"]),
        "diagnostic_only": bool(
            IRG207_INPUT["material"]["model_status"]["diagnostic_only"]
        ),
        "parameter_status": str(IRG207_INPUT["material"]["parameter_status"]),
    }
    return metadata
...
def save_irg207_plot_variables(
    cases: Sequence[CaseDict],
    output_dir: Path,
    ...
    """Save long-format IRG207 plot data as CSV tables only."""
    ...
    print("\nSaving labeled IRG207 plot variables ...")
    for case in cases:
        short = str(case["short"])
        ...
        component_manifest[short] = {
            "used_by": ["IRG207_fig2_NIR.png", "IRG207_fig4_NIR_scaling.png"],
            "arrays": {},
        }
        ...
        component_gamma = np.asarray(
            gamma_irg207_reference_from_intensity_wcm2(
                component["I_wcm2"],
                wavelength_um=float(case["wavelength_um"]),
            ),
            dtype=float,
        )
        component_gamma_name = f"component__{short}__gamma_irg207_reference"
        arrays[component_gamma_name] = component_gamma
        component_manifest[short]["arrays"]["gamma_irg207_reference"] = component_gamma_name
            ...
            component_rows.append(
                {
                    **metadata,
                    ...
                    "gamma_irg207_reference": float(component_gamma[index]),
        ...
        total_gamma_name = f"total_comparison__{short}__gamma_irg207_reference"
        arrays[total_gamma_name] = total_gamma
        total_manifest[short]["arrays"]["gamma_irg207_reference"] = total_gamma_name
            ...
            total_rows.append(
                {
                    **metadata,
                    ...
                    "point_index": int(index),
                    "I_wcm2": float(intensity),
                    "gamma_irg207_reference": float(total_gamma[index]),
    ...
    manifest = {
        "description": "Labeled variables for IRG207 Keldysh and avalanche-ionization plots.",
        ...
        "known_limitations": list(
            IRG207_INPUT["material"]["known_limitations"]
        ),
        "units": {
            "I_wcm2": "W/cm^2",
            "F0_jcm2": "J/cm^2",
            "beam_diameter_a_um": "um",
            "beam_diameter_b_um": "um",
            ...
            "ne_grid_cm3": "cm^-3",
            "gamma_irg207_reference": "dimensionless",
        },
        "files": {
            "component_scaling_csv": "irg207_component_scaling_long.csv",
            "total_comparison_csv": "irg207_total_comparison_long.csv",
            "surface3d_csv": "irg207_surface3d_long.csv",
        },
        "component_scaling": component_manifest,
        "total_comparison": total_manifest,
        "surface3d": surface_manifest,
    }
    _write_csv(output_dir / "irg207_component_scaling_long.csv", component_rows)
    _write_csv(output_dir / "irg207_total_comparison_long.csv", total_rows)
    _write_csv(output_dir / "irg207_surface3d_long.csv", surface_rows)
    print(f"Saved IRG207 CSV data to {output_dir}")
...
def solve_and_display_summary(cases: Sequence[CaseDict]) -> None:
    """Solve the IRG207 cases and display the summary table."""
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run the reconciled IRG207 Keldysh + avalanche model and "
    ...
    parser.add_argument(
        ...
        default="all",
        help=(
            "all: all IRG207 plots and table; summary: first/last graph sets "
            "and table; figures: IRG207 Figs. 1-4; 3d: total-ionization "
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Save every currently enabled IRG207 figure and labeled numerical variables "
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        choices=range(len(IRG207_INPUT["cases"])),
        metavar="INDEX",
        help=(
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    cases = get_cases()
    material_input = IRG207_INPUT["material"]
    print(f"Running full IRG207 workflow ({len(cases)} enabled case(s))")
    print("Source-paper convention: E = sqrt(I/(c n eps0)); gamma uses I, not 2I")
    optical_index = material_input["optical_index"]
    kappa_value = float(optical_index["kappa"])
    if np.isfinite(kappa_value):
        print(
            "Optical index input: "
            f"n+i*kappa = {optical_index['n_real']}+{kappa_value}i; "
            "kappa is recorded but not included in this calculation."
        )
    else:
        print(
            f"Optical index input: n = {optical_index['n_real']}; "
            "kappa is not specified and is not included in this calculation."
        )
    ...
    for case in cases:
        I0_wcm2 = case_reference_peak_intensity_wcm2(case)
        gamma0 = gamma_irg207_reference_from_intensity_wcm2(
            I0_wcm2,
            wavelength_um=float(case["wavelength_um"]),
        )
    ...
    if args.save:
        ...
        plot_irg207_figures(cases, args.points, save_dir)
    # CODEX MODIFICATION START: CLI support for 3D surface plot
    elif args.mode == "3d":
