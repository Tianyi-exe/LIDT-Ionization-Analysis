# KRS5-only Keldysh + avalanche-ionization workflow.
# Standalone full workflow ported from Keldsyh_II_NaCl.py.

# %%
# Cell 0
"""
Reconciled Keldysh + avalanche ionization model for KRS-5.
...
Default workflow
----------------
The default ``--mode all`` execution produces the KRS-5 0.8-um reference
case specified in ``KRS5_Keldysh_Parameter.docx``. The full NaCl/BaF2
...
# CODEX MODIFICATION START: constants from KRS5 verified reference / CODATA
C0 = 299792458.0
EPS0 = 8.8541878188e-12
E_CHARGE = 1.602176634e-19
HBAR = 1.054571817e-34
ME0 = 9.1093837139e-31
# CODEX MODIFICATION END: constants from KRS5 verified reference / CODATA
...
# Source: KRS5_Keldysh_Parameter.docx.
...
# NaCl/BaF2 calculation logic and are diagnostic KRS-5 inputs, not a
# complete Ti3+ electronic-structure model.
KRS5_INPUT: Dict[str, Any] = {
    "reference_file": "../input_parameters/KRS5_Keldysh_Parameter.docx",
    "material": {
        "name": 'KRS-5 (TlBr-TlI mixed crystal)',
        "short": 'KRS5',
        "mat_flag": 1,
        "bandgap_ev": 2.5,
        "reduced_mass": {
        ...
        "parameter_status": 'The 800-nm real index is a Cauchy interpolation and the 2.50 eV gap is provisional. The extinction coefficient is unavailable; this Re(n)-only result is diagnostic.',
        "n2_m2_per_w": np.nan,
        "optical_index": {
            "wavelength_um": 0.8,
            "n_real": 2.4908831176,
            ...
            "note": 'Cauchy interpolation from tabulated 0.54, 1.00, and 1.50 um KRS-5 indices.',
        },
        "model_status": {"diagnostic_only": True},
        "known_limitations": (
            "The 800-nm index is interpolated rather than a direct measured entry, and kappa is not specified.",
            "The 2.50 eV band gap is a provisional placeholder for composition-dependent KRS-5.",
    ...
    "cases": [
        {
            "name": r"KRS-5, 0.8 $\mu$m",
            "short": "KRS5_NIR",
            "material": 'KRS-5 (TlBr-TlI mixed crystal)',
            "region": "NIR",
            "mat_flag": 1,
            "wavelength_um": 0.8,
            "tau_fs": 100.0,
            ...
            "input_note": (
                "F, a, and b follow KRS5_Keldysh_Parameter.docx. "
...
# CODEX MODIFICATION START: KRS5-only material parameters
def material_flag1(
    mat_flag: int,
    wavelength_um: float,
) -> Tuple[float, float, float, float, float]:
    """Return the documented 800-nm KRS-5 material parameters."""

    lam = float(wavelength_um)
    if lam <= 0.0:
        raise ValueError("wavelength_um must be positive.")
    if mat_flag != 1:
        raise ValueError("mat_flag must be 1 for KRS-5.")

    material_input = KRS5_INPUT["material"]
    ...
    if not valid_min <= lam <= valid_max:
        raise ValueError(
            f"KRS-5 optical constants are specified only for "
...
# CODEX MODIFICATION END: KRS5-only material parameters


# CODEX MODIFICATION START: KRS5-only case definitions
def get_cases() -> List[CaseDict]:
    """
    Return the currently enabled KRS-5 cases.

    Returns
    -------
    list of dict
        KRS-5 material/laser case dictionaries. The list-based design
        retains the complete multi-case framework even though one documented
        case is enabled by default.
    """

    cases: List[CaseDict] = []
    for case_input in KRS5_INPUT["cases"]:
        ...
        cases.append(case)
    return cases
# CODEX MODIFICATION END: KRS5-only case definitions
...
def display_summary_table(rows: Sequence[Dict[str, Any]]) -> None:
    ...
    # CODEX MODIFICATION START: display optional KRS5 reference fields cleanly
    ...
    # CODEX MODIFICATION END: display optional KRS5 reference fields cleanly
...
# ============================================================
# KRS5-reference Keldysh parameter axis
# ============================================================

# CODEX MODIFICATION START: KRS5-reference Keldysh parameter axis
def gamma_krs5_reference_from_intensity_wcm2(
    I_wcm2: ArrayLike,
    wavelength_um: float,
) -> ArrayLike:
    """
    Evaluate the Keldysh parameter using KRS5 as the reference material.
    ...
    if np.any(valid):
        ...
        n_krs5, _n2, Eg_krs5_J, mred_krs5, _trans = material_flag1(1, wavelength_um)
        omega = 2.0 * np.pi * C0 / (wavelength_um * 1.0e-6)
        with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
            gamma_valid = (omega / E_CHARGE) * np.sqrt(
                (mred_krs5 * C0 * n_krs5 * EPS0 * Eg_krs5_J)
                / I_wm2
            )
...
def intensity_wcm2_from_gamma_krs5_reference(
    gamma: ArrayLike,
    wavelength_um: float,
) -> ArrayLike:
    """
    Convert a KRS5-reference Keldysh parameter to intensity in W/cm^2.
    """
...
def add_krs5_gamma_top_axis(
    ax: plt.Axes,
    wavelength_um: float,
    gamma_ticks: Tuple[float, ...],
) -> None:
    """
    Add a KRS5-reference Keldysh-parameter axis above an intensity axis.
    """
    ...
    tick_positions = np.asarray(
        intensity_wcm2_from_gamma_krs5_reference(
    ...
    ax_top.set_xlabel(r"Keldysh parameter $\gamma$ (KRS5 reference)")
    ax_top.tick_params(axis="x", which="both", direction="in")
# CODEX MODIFICATION END: KRS5-reference Keldysh parameter axis
...
def plot_total_ionization_nir_lwir_comparison_with_gamma_axis(
    ...
    """
    Plot peak total ionization rate for the enabled KRS5 wavelength regimes.

    Each panel includes a KRS5-reference Keldysh-parameter top axis and a
    ...
    if not enabled_regimes:
        raise ValueError("At least one KRS5 NIR or LWIR case is required.")
    panel_labels = {"NIR": "(a) NIR", "LWIR": "(b) LWIR"}
    gamma_ticks_by_regime = {
        "NIR": (10.0, 3.0, 1.0, 0.3),
        "LWIR": (1.0, 0.3, 0.1),
    }
    material_order = {"KRS5": 0}
...
def plot_material_figure_4_style(
    ...
    save_or_show(
        fig,
        save_dir,
        "KRS5_fig4_NIR_scaling.png",
    )


# CODEX MODIFICATION START: KRS5-only plotting workflow
def plot_krs5_figures(
    cases: Sequence[CaseDict],
    n_intensity_points: int = 70,
    save_dir: Optional[Path] = None,
) -> None:
    """Generate all Figs. 1-4 for KRS5."""

    print("\n============================================================")
    print("Generating KRS5 plots")
...
# CODEX MODIFICATION END: KRS5-only plotting workflow
...
def plot_total_ionization_3d_surface_grid(
    ...
    """Plot all KRS5 total-ionization 3D surfaces in one figure."""
    ...
    fig.suptitle(
        rf"Total ionization surfaces: KRS5 {enabled_regions}",
        fontsize=15,
    )
    save_or_show(
        fig=fig,
        save_dir=save_dir,
        filename=f"05_total_ionization_3d_KRS5_{'_'.join(str(case['region']) for case in cases)}.png",
...
def _case_export_metadata(case: CaseDict) -> Dict[str, Any]:
    ...
    metadata: Dict[str, Any] = {
        ...
        "reduced_mass_definition": str(
            KRS5_INPUT["material"]["reduced_mass"]["definition"]
        ),
        "kappa_recorded_not_used": (
            float(KRS5_INPUT["material"]["optical_index"]["kappa"])
            if np.isfinite(float(KRS5_INPUT["material"]["optical_index"]["kappa"]))
            else None
        ),
        "transmission_factor": float(trans),
        "reference_file": str(KRS5_INPUT["reference_file"]),
        "diagnostic_only": bool(
            KRS5_INPUT["material"]["model_status"]["diagnostic_only"]
        ),
        "parameter_status": str(KRS5_INPUT["material"]["parameter_status"]),
    }
    return metadata
...
def save_krs5_plot_variables(
    ...
    """Save long-format KRS-5 plot data as CSV tables only."""
    ...
    print("\nSaving labeled KRS5 plot variables ...")
    for case in cases:
        short = str(case["short"])
        ...
        component_manifest[short] = {
            "used_by": ["KRS5_fig2_NIR.png", "KRS5_fig4_NIR_scaling.png"],
            "arrays": {},
        }
        ...
        component_gamma = np.asarray(
            gamma_krs5_reference_from_intensity_wcm2(
                component["I_wcm2"],
                wavelength_um=float(case["wavelength_um"]),
            ),
            dtype=float,
        )
        component_gamma_name = f"component__{short}__gamma_krs5_reference"
        arrays[component_gamma_name] = component_gamma
        component_manifest[short]["arrays"]["gamma_krs5_reference"] = component_gamma_name
            ...
            component_rows.append(
                {
                    **metadata,
                    ...
                    "gamma_krs5_reference": float(component_gamma[index]),
        ...
        total_gamma_name = f"total_comparison__{short}__gamma_krs5_reference"
        arrays[total_gamma_name] = total_gamma
        total_manifest[short]["arrays"]["gamma_krs5_reference"] = total_gamma_name
            ...
            total_rows.append(
                {
                    **metadata,
                    ...
                    "point_index": int(index),
                    "I_wcm2": float(intensity),
                    "gamma_krs5_reference": float(total_gamma[index]),
    ...
    manifest = {
        "description": "Labeled variables for KRS-5 Keldysh and avalanche-ionization plots.",
        ...
        "known_limitations": list(
            KRS5_INPUT["material"]["known_limitations"]
        ),
        "units": {
            "I_wcm2": "W/cm^2",
            "F0_jcm2": "J/cm^2",
            ...
            "ne_grid_cm3": "cm^-3",
            "gamma_krs5_reference": "dimensionless",
        },
        "files": {
            "component_scaling_csv": "krs5_component_scaling_long.csv",
            "total_comparison_csv": "krs5_total_comparison_long.csv",
            "surface3d_csv": "krs5_surface3d_long.csv",
        },
        "component_scaling": component_manifest,
        "total_comparison": total_manifest,
        "surface3d": surface_manifest,
    }
    _write_csv(output_dir / "krs5_component_scaling_long.csv", component_rows)
    _write_csv(output_dir / "krs5_total_comparison_long.csv", total_rows)
    _write_csv(output_dir / "krs5_surface3d_long.csv", surface_rows)
    print(f"Saved KRS-5 CSV data to {output_dir}")
...
def solve_and_display_summary(cases: Sequence[CaseDict]) -> None:
    """Solve the KRS5 cases and display the summary table."""
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run the reconciled KRS5 Keldysh + avalanche model and "
    ...
    parser.add_argument(
        ...
        default="all",
        help=(
            "all: all KRS5 plots and table; summary: first/last graph sets "
            "and table; figures: KRS5 Figs. 1-4; 3d: total-ionization "
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Save every currently enabled KRS5 figure and labeled numerical variables "
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        choices=range(len(KRS5_INPUT["cases"])),
        metavar="INDEX",
        help=(
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    cases = get_cases()
    material_input = KRS5_INPUT["material"]
    print(f"Running full KRS-5 workflow ({len(cases)} enabled case(s))")
    ...
    for case in cases:
        I0_wcm2 = case_reference_peak_intensity_wcm2(case)
        gamma0 = gamma_krs5_reference_from_intensity_wcm2(
            I0_wcm2,
            wavelength_um=float(case["wavelength_um"]),
        )
    ...
    if args.save:
        ...
        plot_krs5_figures(cases, args.points, save_dir)
    # CODEX MODIFICATION START: CLI support for 3D surface plot
    elif args.mode == "3d":
