# TiSapphire-only Keldysh + avalanche-ionization workflow.
# Standalone full workflow ported from Keldsyh_II_NaCl.py.

# %%
# Cell 0
"""
Reconciled Keldysh + avalanche ionization model for Ti:Sapphire.
...
Default workflow
----------------
The default ``--mode all`` execution produces the Ti:Sapphire 0.8-um reference
case specified in ``TiSapphire_Keldysh_Parameter.docx``. The full NaCl/BaF2
plot, ODE, multi-case, 3D-surface, CSV, and saved-variable framework is retained.
Only the single case documented for this material is enabled by default.
"""
...
# CODEX MODIFICATION START: constants from TiSapphire verified reference / CODATA
C0 = 299792458.0
EPS0 = 8.8541878188e-12
E_CHARGE = 1.602176634e-19
HBAR = 1.054571817e-34
ME0 = 9.1093837139e-31
# CODEX MODIFICATION END: constants from TiSapphire verified reference / CODATA
...
# Source: TiSapphire_Keldysh_Parameter.docx.
# The input document uses the Al2O3 host gap and a temporary electron-hole
# reduced effective mass. These are retained for consistency with the
# NaCl/BaF2 calculation logic and are diagnostic Ti:Sapphire inputs, not a
# complete Ti3+ electronic-structure model.
TI_SAPPHIRE_INPUT: Dict[str, Any] = {
    "reference_file": "../input_parameters/TiSapphire_Keldysh_Parameter.docx",
    "material": {
        "name": "Ti:Sapphire (Ti3+:Al2O3)",
        "short": "TiSapphire",
        "mat_flag": 1,
        "bandgap_ev": 8.8,
        "reduced_mass": {
            "electron_hole_reduced_effective_mass_over_me": 1.0,
            "definition": "electron-hole reduced effective mass",
            "status": "Temporary project input; not a measured Ti:Sapphire constant.",
        },
        "parameter_status": (
            "Diagnostic host-material Keldysh reference with temporary mred = 1.0 m0. "
            "Supply Ti concentration, "
            "inversion, orientation, polarization, and a band-specific "
            "electron-hole reduced effective mass for a material-specific simulation."
        ),
        "n2_m2_per_w": np.nan,
        "optical_index": {
            "wavelength_um": 0.8,
            "n_real": 1.76,
            "kappa": 0.0,
            "valid_wavelength_um": (0.79, 0.81),
            "note": "Index supplied by the Ti:Sapphire input document.",
        },
        "model_status": {"diagnostic_only": True},
        "known_limitations": (
            "Uses the Al2O3 host gap and a reference electron-hole reduced mass; "
            "Ti3+ absorption, gain, inversion, and crystal anisotropy are omitted.",
            "The legacy Drude/avalanche term reuses the reduced mass; a future "
            "material-specific model should supply a conductivity mass separately.",
            "Carrier depletion, saturation, recombination, heating, and propagation "
            "are not included.",
            "The inherited qfun termination should be benchmarked independently "
            "before nonmonotonic rate features are assigned physical meaning.",
        ),
    },
    "cases": [
        {
            "name": r"Ti:Sapphire, 0.8 $\mu$m",
            "short": "TiSapphire_NIR",
            ...
            "F0_jcm2": 0.61,
            "beam_radius_a_um": 162.1,
            "beam_radius_b_um": 152.4,
            "beam_radius_definition": "1/e^2 intensity radius",
            "fluence_basis": "surface incident",
            "input_note": (
                "The surface-incident peak fluence and 1/e^2 intensity radii are "
                "from TiSapphire_Keldysh_Parameter.docx. Reflection is neglected, "
                "so the surface fluence is used directly as the zero-dimensional "
                "effective fluence."
            ),
        },
    ],
...
# CODEX MODIFICATION START: TiSapphire-only material parameters
def material_flag1(
    mat_flag: int,
    wavelength_um: float,
) -> Tuple[float, float, float, float, float]:
    """
    Return the documented 800-nm Ti:Sapphire material parameters.

    Parameters
    ----------
    mat_flag:
        Material selector. This standalone script uses only 1 for Ti:Sapphire.
    wavelength_um:
    ...
    n2:
        ...
        Ti:Sapphire Keldysh-parameter reference document.
    Eg_J:
    ...
    ------
    ValueError
        If the material flag is not 1 or the fixed-index wavelength range is
        exceeded.
    """
    ...
    if mat_flag != 1:
        raise ValueError("mat_flag must be 1 for Ti:Sapphire.")

    material_input = TI_SAPPHIRE_INPUT["material"]
    bandgap_ev = float(material_input["bandgap_ev"])
    reduced_mass = material_input["reduced_mass"]
    mred = float(reduced_mass["electron_hole_reduced_effective_mass_over_me"]) * ME0
    optical_index = material_input["optical_index"]
    valid_min, valid_max = optical_index["valid_wavelength_um"]
    if not valid_min <= lam <= valid_max:
        raise ValueError(
            f"Ti:Sapphire n={optical_index['n_real']} is specified only for "
            f"{valid_min}-{valid_max} um; "
            f"received {lam} um."
        )
    n0 = float(optical_index["n_real"])
    n2 = float(material_input["n2_m2_per_w"])
    if not np.isfinite(n0) or n0 <= 0.0:
        raise ValueError(f"Nonphysical refractive index n={n0!r}.")
...
# CODEX MODIFICATION END: TiSapphire-only material parameters


# CODEX MODIFICATION START: TiSapphire-only case definitions
def get_cases() -> List[CaseDict]:
    """
    Return the currently enabled Ti:Sapphire cases.

    Returns
    -------
    list of dict
        Ti:Sapphire material/laser case dictionaries. The list-based design
        retains the complete multi-case framework even though one documented
        case is enabled by default.
    """

    cases: List[CaseDict] = []
    for case_input in TI_SAPPHIRE_INPUT["cases"]:
        ...
        cases.append(case)
    return cases
# CODEX MODIFICATION END: TiSapphire-only case definitions
...
def case_marker_peak_intensity_wcm2(case: CaseDict) -> Optional[Tuple[float, str]]:
    ...
    if I_from_fluence is not None:
        return I_from_fluence, r"$I_0$ from fluence"
    if case.get("reference_I0_wcm2") is not None:
        ...
        rows.append(
            {
                ...
                "tau_fs": case["tau_fs"],
                "F0_Jcm2": case.get("F0_jcm2"),
                "beam_radius_a_um": case.get("beam_radius_a_um"),
                "beam_radius_b_um": case.get("beam_radius_b_um"),
...
def display_summary_table(rows: Sequence[Dict[str, Any]]) -> None:
    ...
    # CODEX MODIFICATION START: display optional TiSapphire reference fields cleanly
    ...
    # CODEX MODIFICATION END: display optional TiSapphire reference fields cleanly
...
# ============================================================
# TiSapphire-reference Keldysh parameter axis
# ============================================================

# CODEX MODIFICATION START: TiSapphire-reference Keldysh parameter axis
def gamma_ti_sapphire_reference_from_intensity_wcm2(
    I_wcm2: ArrayLike,
    wavelength_um: float,
) -> ArrayLike:
    """
    Evaluate the Keldysh parameter using TiSapphire as the reference material.
    ...
    if np.any(valid):
        ...
        n_ti_sapphire, _n2, Eg_ti_sapphire_J, mred_ti_sapphire, _trans = material_flag1(1, wavelength_um)
        omega = 2.0 * np.pi * C0 / (wavelength_um * 1.0e-6)
        with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
            gamma_valid = (omega / E_CHARGE) * np.sqrt(
                (mred_ti_sapphire * C0 * n_ti_sapphire * EPS0 * Eg_ti_sapphire_J)
                / I_wm2
            )
...
def intensity_wcm2_from_gamma_ti_sapphire_reference(
    gamma: ArrayLike,
    wavelength_um: float,
) -> ArrayLike:
    """
    Convert a TiSapphire-reference Keldysh parameter to intensity in W/cm^2.
    """
...
def add_ti_sapphire_gamma_top_axis(
    ax: plt.Axes,
    wavelength_um: float,
    gamma_ticks: Tuple[float, ...],
) -> None:
    """
    Add a TiSapphire-reference Keldysh-parameter axis above an intensity axis.
    """
    ...
    tick_positions = np.asarray(
        intensity_wcm2_from_gamma_ti_sapphire_reference(
    ...
    ax_top.set_xlabel(r"Keldysh parameter $\gamma$ (TiSapphire reference)")
    ax_top.tick_params(axis="x", which="both", direction="in")
# CODEX MODIFICATION END: TiSapphire-reference Keldysh parameter axis
...
def plot_total_ionization_nir_lwir_comparison_with_gamma_axis(
    ...
    """
    Plot peak total ionization rate for the enabled TiSapphire wavelength regimes.

    Each panel includes a TiSapphire-reference Keldysh-parameter top axis and a
    ...
    if not enabled_regimes:
        raise ValueError("At least one TiSapphire NIR or LWIR case is required.")
    panel_labels = {"NIR": "(a) NIR", "LWIR": "(b) LWIR"}
    gamma_ticks_by_regime = {
        "NIR": (10.0, 3.0, 1.0, 0.3),
        "LWIR": (1.0, 0.3, 0.1),
    }
    material_order = {"TiSapphire": 0}
    ...
    for ax, regime in zip(axes, enabled_regimes):
        ...
        add_ti_sapphire_gamma_top_axis(
            ax=ax,
            wavelength_um=wavelength_um,
...
def plot_case_figure_1_style(
    ...
    save_or_show(
        fig,
        save_dir,
        f"TiSapphire_fig1_{case['region']}.png",
    )
...
def plot_case_figure_2_style(
    ...
    save_or_show(
        fig,
        save_dir,
        f"TiSapphire_fig2_{case['region']}.png",
    )
...
def plot_case_figure_3_style(
    ...
    save_or_show(
        fig,
        save_dir,
        f"TiSapphire_fig3_{case['region']}.png",
    )
...
def plot_material_figure_4_style(
    ...
    save_or_show(
        fig,
        save_dir,
        "TiSapphire_fig4_NIR_scaling.png",
    )


# CODEX MODIFICATION START: TiSapphire-only plotting workflow
def plot_ti_sapphire_figures(
    cases: Sequence[CaseDict],
    n_intensity_points: int = 70,
    save_dir: Optional[Path] = None,
) -> None:
    """Generate all Figs. 1-4 for TiSapphire."""

    print("\n============================================================")
    print("Generating TiSapphire plots")
...
# CODEX MODIFICATION END: TiSapphire-only plotting workflow
...
def plot_total_ionization_3d_surface_grid(
    ...
    """Plot all TiSapphire total-ionization 3D surfaces in one figure."""
    ...
    fig.suptitle(
        rf"Total ionization surfaces: TiSapphire {enabled_regions}",
        fontsize=15,
    )
    save_or_show(
        fig=fig,
        save_dir=save_dir,
        filename=f"05_total_ionization_3d_TiSapphire_{'_'.join(str(case['region']) for case in cases)}.png",
...
def _case_export_metadata(case: CaseDict) -> Dict[str, Any]:
    ...
    metadata: Dict[str, Any] = {
        ...
        "beam_radius_a_um": float(case["beam_radius_a_um"]),
        "beam_radius_b_um": float(case["beam_radius_b_um"]),
        "beam_radius_definition": str(case["beam_radius_definition"]),
        "fluence_basis": str(case["fluence_basis"]),
        ...
        "reduced_mass_definition": str(
            TI_SAPPHIRE_INPUT["material"]["reduced_mass"]["definition"]
        ),
        "kappa_recorded_not_used": float(
            TI_SAPPHIRE_INPUT["material"]["optical_index"]["kappa"]
        ),
        "transmission_factor": float(trans),
        "reference_file": str(TI_SAPPHIRE_INPUT["reference_file"]),
        "diagnostic_only": bool(
            TI_SAPPHIRE_INPUT["material"]["model_status"]["diagnostic_only"]
        ),
        "parameter_status": str(TI_SAPPHIRE_INPUT["material"]["parameter_status"]),
    }
    return metadata
...
def save_ti_sapphire_plot_variables(
    cases: Sequence[CaseDict],
    output_dir: Path,
    ...
    """Save long-format Ti:Sapphire plot data as CSV tables only."""
    ...
    print("\nSaving labeled TiSapphire plot variables ...")
    for case in cases:
        short = str(case["short"])
        metadata = _case_export_metadata(case)
        case_metadata[short] = metadata
        # Resolve the reference point from the measured surface fluence and
        # Gaussian temporal pulse duration stored in the case input.
        ...
        component_manifest[short] = {
            "used_by": ["TiSapphire_fig2_NIR.png", "TiSapphire_fig4_NIR_scaling.png"],
            "arrays": {},
        }
        ...
        component_gamma = np.asarray(
            gamma_ti_sapphire_reference_from_intensity_wcm2(
                component["I_wcm2"],
                wavelength_um=float(case["wavelength_um"]),
            ),
            dtype=float,
        )
        component_gamma_name = f"component__{short}__gamma_ti_sapphire_reference"
        arrays[component_gamma_name] = component_gamma
        component_manifest[short]["arrays"]["gamma_ti_sapphire_reference"] = component_gamma_name
            ...
            component_rows.append(
                {
                    **metadata,
                    ...
                    "gamma_ti_sapphire_reference": float(component_gamma[index]),
        ...
        total_gamma_name = f"total_comparison__{short}__gamma_ti_sapphire_reference"
        arrays[total_gamma_name] = total_gamma
        total_manifest[short]["arrays"]["gamma_ti_sapphire_reference"] = total_gamma_name
            ...
            total_rows.append(
                {
                    **metadata,
                    ...
                    "point_index": int(index),
                    "I_wcm2": float(intensity),
                    "gamma_ti_sapphire_reference": float(total_gamma[index]),
    ...
    manifest = {
        "description": "Labeled variables for Ti:Sapphire Keldysh and avalanche-ionization plots.",
        ...
        "known_limitations": list(
            TI_SAPPHIRE_INPUT["material"]["known_limitations"]
        ),
        "units": {
            "I_wcm2": "W/cm^2",
            "F0_jcm2": "J/cm^2",
            ...
            "ne_grid_cm3": "cm^-3",
            "gamma_ti_sapphire_reference": "dimensionless",
        },
        "files": {
            "component_scaling_csv": "ti_sapphire_component_scaling_long.csv",
            "total_comparison_csv": "ti_sapphire_total_comparison_long.csv",
            "surface3d_csv": "ti_sapphire_surface3d_long.csv",
        },
        "component_scaling": component_manifest,
        "total_comparison": total_manifest,
        "surface3d": surface_manifest,
    }
    _write_csv(output_dir / "ti_sapphire_component_scaling_long.csv", component_rows)
    _write_csv(output_dir / "ti_sapphire_total_comparison_long.csv", total_rows)
    _write_csv(output_dir / "ti_sapphire_surface3d_long.csv", surface_rows)
    print(f"Saved Ti:Sapphire CSV data to {output_dir}")
...
def solve_and_display_summary(cases: Sequence[CaseDict]) -> None:
    """Solve the TiSapphire cases and display the summary table."""
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run the reconciled TiSapphire Keldysh + avalanche model and "
    ...
    parser.add_argument(
        ...
        default="all",
        help=(
            "all: all TiSapphire plots and table; summary: first/last graph sets "
            "and table; figures: TiSapphire Figs. 1-4; 3d: total-ionization "
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Save every currently enabled TiSapphire figure and labeled numerical variables "
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        choices=range(len(TI_SAPPHIRE_INPUT["cases"])),
        metavar="INDEX",
        help=(
            "Optional enabled-case index used by --mode 3d. "
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    cases = get_cases()
    material_input = TI_SAPPHIRE_INPUT["material"]
    print(f"Running full Ti:Sapphire workflow ({len(cases)} enabled case(s))")
    print("Source-paper convention: E = sqrt(I/(c n eps0)); gamma uses I, not 2I")
    print(f"Parameter status: {material_input['parameter_status']}")
    if material_input["model_status"]["diagnostic_only"]:
        print("WARNING: diagnostic only; see the recorded known limitations before interpretation.")
    for case in cases:
        I0_wcm2 = case_reference_peak_intensity_wcm2(case)
        gamma0 = gamma_ti_sapphire_reference_from_intensity_wcm2(
            I0_wcm2,
            wavelength_um=float(case["wavelength_um"]),
        )
        print(f"{case['short']}: gamma(I0) = {gamma0:.6g}")
    ...
    if args.save:
        ...
        plot_ti_sapphire_figures(cases, args.points, save_dir)
    # CODEX MODIFICATION START: CLI support for 3D surface plot
    elif args.mode == "3d":
    ...
    if args.save:
        ...
        save_ti_sapphire_plot_variables(
            cases=cases,
