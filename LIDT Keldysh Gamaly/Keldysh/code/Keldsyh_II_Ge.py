# Ge-only Keldysh + avalanche-ionization workflow.
# Standalone full workflow ported from Keldsyh_II_NaCl.py.

# %%
# Cell 0
"""
Reconciled Keldysh + avalanche ionization model for Ge.
...
Default workflow
----------------
The default ``--mode all`` execution produces the Ge 0.8-um reference
case specified in ``Ge_Keldysh_Parameter.docx``. The full NaCl/BaF2
...
# CODEX MODIFICATION START: constants from Ge verified reference / CODATA
C0 = 299792458.0
EPS0 = 8.8541878188e-12
E_CHARGE = 1.602176634e-19
HBAR = 1.054571817e-34
ME0 = 9.1093837139e-31
# CODEX MODIFICATION END: constants from Ge verified reference / CODATA
...
# Source: Ge_Keldysh_Parameter.docx.
...
# NaCl/BaF2 calculation logic and are diagnostic Ge inputs, not a
# complete Ti3+ electronic-structure model.
GE_INPUT: Dict[str, Any] = {
    "reference_file": "../input_parameters/Ge_Keldysh_Parameter.docx",
    "material": {
        "name": 'Germanium (Ge)',
        "short": 'Ge',
        "mat_flag": 1,
        ...
        "parameter_status": 'At 800 nm, hnu is about 1.550 eV and exceeds both the indirect and direct gaps. Ge is absorbing; this Re(n)-only zero-D result is diagnostic and omits one-photon absorption, kappa, reflection, and propagation.',
        "n2_m2_per_w": np.nan,
        "optical_index": {
            "wavelength_um": 0.8,
            "n_real": 4.699,
            ...
            "note": 'Jellison 800-nm crystalline-Ge optical constants.',
        },
        "model_status": {"diagnostic_only": True},
        "known_limitations": (
            "At 800 nm, direct one-photon interband absorption and complex-index propagation are omitted.",
            "The fixed 0.660 eV input is the 300 K indirect gap; valley- and transition-specific masses are not modeled.",
    ...
    "cases": [
        {
            ...
            "input_note": (
                "F, a, and b follow Ge_Keldysh_Parameter.docx. "
...
# CODEX MODIFICATION START: Ge-only material parameters
def material_flag1(
    mat_flag: int,
    wavelength_um: float,
) -> Tuple[float, float, float, float, float]:
    """Return the documented 800-nm Ge material parameters."""

    lam = float(wavelength_um)
    if lam <= 0.0:
        raise ValueError("wavelength_um must be positive.")
    if mat_flag != 1:
        raise ValueError("mat_flag must be 1 for Ge.")

    material_input = GE_INPUT["material"]
    ...
    if not valid_min <= lam <= valid_max:
        raise ValueError(
            f"Ge optical constants are specified only for "
...
# CODEX MODIFICATION END: Ge-only material parameters


# CODEX MODIFICATION START: Ge-only case definitions
def get_cases() -> List[CaseDict]:
    """
    Return the currently enabled Ge cases.

    Returns
    -------
    list of dict
        Ge material/laser case dictionaries. The list-based design
        retains the complete multi-case framework even though one documented
        case is enabled by default.
    """

    cases: List[CaseDict] = []
    for case_input in GE_INPUT["cases"]:
        ...
        cases.append(case)
    return cases
# CODEX MODIFICATION END: Ge-only case definitions
...
def display_summary_table(rows: Sequence[Dict[str, Any]]) -> None:
    ...
    # CODEX MODIFICATION START: display optional Ge reference fields cleanly
    ...
    # CODEX MODIFICATION END: display optional Ge reference fields cleanly
...
# ============================================================
# Ge-reference Keldysh parameter axis
# ============================================================

# CODEX MODIFICATION START: Ge-reference Keldysh parameter axis
def gamma_ge_reference_from_intensity_wcm2(
    I_wcm2: ArrayLike,
    wavelength_um: float,
) -> ArrayLike:
    """
    Evaluate the Keldysh parameter using Ge as the reference material.
    ...
    if np.any(valid):
        ...
        n_ge, _n2, Eg_ge_J, mred_ge, _trans = material_flag1(1, wavelength_um)
        omega = 2.0 * np.pi * C0 / (wavelength_um * 1.0e-6)
        with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
            gamma_valid = (omega / E_CHARGE) * np.sqrt(
                (mred_ge * C0 * n_ge * EPS0 * Eg_ge_J)
                / I_wm2
            )
...
def intensity_wcm2_from_gamma_ge_reference(
    gamma: ArrayLike,
    wavelength_um: float,
) -> ArrayLike:
    """
    Convert a Ge-reference Keldysh parameter to intensity in W/cm^2.
    """
...
def add_ge_gamma_top_axis(
    ax: plt.Axes,
    wavelength_um: float,
    gamma_ticks: Tuple[float, ...],
) -> None:
    """
    Add a Ge-reference Keldysh-parameter axis above an intensity axis.
    """
    ...
    tick_positions = np.asarray(
        intensity_wcm2_from_gamma_ge_reference(
    ...
    ax_top.set_xlabel(r"Keldysh parameter $\gamma$ (Ge reference)")
    ax_top.tick_params(axis="x", which="both", direction="in")
# CODEX MODIFICATION END: Ge-reference Keldysh parameter axis
...
def plot_total_ionization_nir_lwir_comparison_with_gamma_axis(
    ...
    """
    Plot peak total ionization rate for the enabled Ge wavelength regimes.

    Each panel includes a Ge-reference Keldysh-parameter top axis and a
    ...
    if not enabled_regimes:
        raise ValueError("At least one Ge NIR or LWIR case is required.")
...
def plot_material_figure_4_style(
    ...
    save_or_show(
        fig,
        save_dir,
        "Ge_fig4_NIR_scaling.png",
    )


# CODEX MODIFICATION START: Ge-only plotting workflow
def plot_ge_figures(
    cases: Sequence[CaseDict],
    n_intensity_points: int = 70,
    save_dir: Optional[Path] = None,
) -> None:
    """Generate all Figs. 1-4 for Ge."""

    print("\n============================================================")
    print("Generating Ge plots")
...
# CODEX MODIFICATION END: Ge-only plotting workflow
...
def plot_total_ionization_3d_surface_grid(
    ...
    """Plot all Ge total-ionization 3D surfaces in one figure."""
    ...
    fig.suptitle(
        rf"Total ionization surfaces: Ge {enabled_regions}",
        fontsize=15,
    )
    save_or_show(
        fig=fig,
        save_dir=save_dir,
        filename=f"05_total_ionization_3d_Ge_{'_'.join(str(case['region']) for case in cases)}.png",
...
def _case_export_metadata(case: CaseDict) -> Dict[str, Any]:
    ...
    metadata: Dict[str, Any] = {
        ...
        "reduced_mass_definition": str(
            GE_INPUT["material"]["reduced_mass"]["definition"]
        ),
        "kappa_recorded_not_used": (
            float(GE_INPUT["material"]["optical_index"]["kappa"])
            if np.isfinite(float(GE_INPUT["material"]["optical_index"]["kappa"]))
            else None
        ),
        "transmission_factor": float(trans),
        "reference_file": str(GE_INPUT["reference_file"]),
        "diagnostic_only": bool(
            GE_INPUT["material"]["model_status"]["diagnostic_only"]
        ),
        "parameter_status": str(GE_INPUT["material"]["parameter_status"]),
    }
    return metadata
...
def save_ge_plot_variables(
    ...
    """Save long-format Ge plot data as CSV tables only."""
    ...
    print("\nSaving labeled Ge plot variables ...")
    for case in cases:
        short = str(case["short"])
        ...
        component_manifest[short] = {
            "used_by": ["Ge_fig2_NIR.png", "Ge_fig4_NIR_scaling.png"],
            "arrays": {},
        }
        ...
        component_gamma = np.asarray(
            gamma_ge_reference_from_intensity_wcm2(
                component["I_wcm2"],
                wavelength_um=float(case["wavelength_um"]),
            ),
            dtype=float,
        )
        component_gamma_name = f"component__{short}__gamma_ge_reference"
        arrays[component_gamma_name] = component_gamma
        component_manifest[short]["arrays"]["gamma_ge_reference"] = component_gamma_name
            ...
            component_rows.append(
                {
                    **metadata,
                    ...
                    "gamma_ge_reference": float(component_gamma[index]),
        ...
        total_gamma_name = f"total_comparison__{short}__gamma_ge_reference"
        arrays[total_gamma_name] = total_gamma
        total_manifest[short]["arrays"]["gamma_ge_reference"] = total_gamma_name
            ...
            total_rows.append(
                {
                    **metadata,
                    ...
                    "point_index": int(index),
                    "I_wcm2": float(intensity),
                    "gamma_ge_reference": float(total_gamma[index]),
    ...
    manifest = {
        "description": "Labeled variables for Ge Keldysh and avalanche-ionization plots.",
        ...
        "known_limitations": list(
            GE_INPUT["material"]["known_limitations"]
        ),
        "units": {
            "I_wcm2": "W/cm^2",
            "F0_jcm2": "J/cm^2",
            ...
            "ne_grid_cm3": "cm^-3",
            "gamma_ge_reference": "dimensionless",
        },
        "files": {
            "component_scaling_csv": "ge_component_scaling_long.csv",
            "total_comparison_csv": "ge_total_comparison_long.csv",
            "surface3d_csv": "ge_surface3d_long.csv",
        },
        "component_scaling": component_manifest,
        "total_comparison": total_manifest,
        "surface3d": surface_manifest,
    }
    _write_csv(output_dir / "ge_component_scaling_long.csv", component_rows)
    _write_csv(output_dir / "ge_total_comparison_long.csv", total_rows)
    _write_csv(output_dir / "ge_surface3d_long.csv", surface_rows)
    print(f"Saved Ge CSV data to {output_dir}")
...
def solve_and_display_summary(cases: Sequence[CaseDict]) -> None:
    """Solve the Ge cases and display the summary table."""
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run the reconciled Ge Keldysh + avalanche model and "
    ...
    parser.add_argument(
        ...
        default="all",
        help=(
            "all: all Ge plots and table; summary: first/last graph sets "
            "and table; figures: Ge Figs. 1-4; 3d: total-ionization "
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Save every currently enabled Ge figure and labeled numerical variables "
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        choices=range(len(GE_INPUT["cases"])),
        metavar="INDEX",
        help=(
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    cases = get_cases()
    material_input = GE_INPUT["material"]
    print(f"Running full Ge workflow ({len(cases)} enabled case(s))")
    ...
    for case in cases:
        I0_wcm2 = case_reference_peak_intensity_wcm2(case)
        gamma0 = gamma_ge_reference_from_intensity_wcm2(
            I0_wcm2,
            wavelength_um=float(case["wavelength_um"]),
        )
    ...
    if args.save:
        ...
        plot_ge_figures(cases, args.points, save_dir)
    # CODEX MODIFICATION START: CLI support for 3D surface plot
    elif args.mode == "3d":
