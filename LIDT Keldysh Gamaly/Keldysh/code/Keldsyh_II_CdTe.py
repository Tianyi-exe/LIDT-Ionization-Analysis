# CdTe-only Keldysh + avalanche-ionization workflow.
# Standalone full workflow ported from Keldsyh_II_NaCl.py.

# %%
# Cell 0
"""
Reconciled Keldysh + avalanche ionization model for CdTe.
...
Default workflow
----------------
The default ``--mode all`` execution produces the CdTe 0.8-um reference
case specified in ``CdTe_Keldysh_Parameter.docx``. The full NaCl/BaF2
...
# CODEX MODIFICATION START: constants from CdTe verified reference / CODATA
C0 = 299792458.0
EPS0 = 8.8541878188e-12
E_CHARGE = 1.602176634e-19
HBAR = 1.054571817e-34
ME0 = 9.1093837139e-31
# CODEX MODIFICATION END: constants from CdTe verified reference / CODATA
...
# Source: CdTe_Keldysh_Parameter.docx.
...
# NaCl/BaF2 calculation logic and are diagnostic CdTe inputs, not a
# complete Ti3+ electronic-structure model.
CDTE_INPUT: Dict[str, Any] = {
    "reference_file": "../input_parameters/CdTe_Keldysh_Parameter.docx",
    "material": {
        ...
        "parameter_status": 'At 800 nm, hnu is about 1.550 eV and is slightly above the 300 K direct gap. One-photon interband absorption is expected; this Re(n)-only zero-D result is diagnostic.',
        "n2_m2_per_w": np.nan,
        "optical_index": {
            "wavelength_um": 0.8,
            "n_real": 2.876,
            ...
            "note": 'Crystran/Palik real-index entry at 800 nm; kappa is not supplied.',
        },
        "model_status": {"diagnostic_only": True},
        "known_limitations": (
            "At 800 nm, one-photon interband absorption and complex-index propagation are omitted.",
            "No surface fluence is present in the current input document; the documented 1e13 W/cm^2 reference intensity is used.",
    ...
    "cases": [
        {
            ...
            "reference_I0_wcm2": 10000000000000.0,
            "F0_jcm2": None,
            ...
            "input_note": (
                "F, a, and b follow CdTe_Keldysh_Parameter.docx. "
...
# CODEX MODIFICATION START: CdTe-only material parameters
def material_flag1(
    mat_flag: int,
    wavelength_um: float,
) -> Tuple[float, float, float, float, float]:
    """Return the documented 800-nm CdTe material parameters."""

    lam = float(wavelength_um)
    if lam <= 0.0:
        raise ValueError("wavelength_um must be positive.")
    if mat_flag != 1:
        raise ValueError("mat_flag must be 1 for CdTe.")

    material_input = CDTE_INPUT["material"]
    ...
    if not valid_min <= lam <= valid_max:
        raise ValueError(
            f"CdTe optical constants are specified only for "
...
# CODEX MODIFICATION END: CdTe-only material parameters


# CODEX MODIFICATION START: CdTe-only case definitions
def get_cases() -> List[CaseDict]:
    """
    Return the currently enabled CdTe cases.

    Returns
    -------
    list of dict
        CdTe material/laser case dictionaries. The list-based design
        retains the complete multi-case framework even though one documented
        case is enabled by default.
    """

    cases: List[CaseDict] = []
    for case_input in CDTE_INPUT["cases"]:
        ...
        cases.append(case)
    return cases
# CODEX MODIFICATION END: CdTe-only case definitions
...
def display_summary_table(rows: Sequence[Dict[str, Any]]) -> None:
    ...
    # CODEX MODIFICATION START: display optional CdTe reference fields cleanly
    ...
    # CODEX MODIFICATION END: display optional CdTe reference fields cleanly
...
# ============================================================
# CdTe-reference Keldysh parameter axis
# ============================================================

# CODEX MODIFICATION START: CdTe-reference Keldysh parameter axis
def gamma_cdte_reference_from_intensity_wcm2(
    I_wcm2: ArrayLike,
    wavelength_um: float,
) -> ArrayLike:
    """
    Evaluate the Keldysh parameter using CdTe as the reference material.
    ...
    if np.any(valid):
        ...
        n_cdte, _n2, Eg_cdte_J, mred_cdte, _trans = material_flag1(1, wavelength_um)
        omega = 2.0 * np.pi * C0 / (wavelength_um * 1.0e-6)
        with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
            gamma_valid = (omega / E_CHARGE) * np.sqrt(
                (mred_cdte * C0 * n_cdte * EPS0 * Eg_cdte_J)
                / I_wm2
            )
...
def intensity_wcm2_from_gamma_cdte_reference(
    gamma: ArrayLike,
    wavelength_um: float,
) -> ArrayLike:
    """
    Convert a CdTe-reference Keldysh parameter to intensity in W/cm^2.
    """
...
def add_cdte_gamma_top_axis(
    ax: plt.Axes,
    wavelength_um: float,
    gamma_ticks: Tuple[float, ...],
) -> None:
    """
    Add a CdTe-reference Keldysh-parameter axis above an intensity axis.
    """
    ...
    tick_positions = np.asarray(
        intensity_wcm2_from_gamma_cdte_reference(
    ...
    ax_top.set_xlabel(r"Keldysh parameter $\gamma$ (CdTe reference)")
    ax_top.tick_params(axis="x", which="both", direction="in")
# CODEX MODIFICATION END: CdTe-reference Keldysh parameter axis
...
def plot_total_ionization_nir_lwir_comparison_with_gamma_axis(
    ...
    """
    Plot peak total ionization rate for the enabled CdTe wavelength regimes.

    Each panel includes a CdTe-reference Keldysh-parameter top axis and a
    ...
    if not enabled_regimes:
        raise ValueError("At least one CdTe NIR or LWIR case is required.")
    panel_labels = {"NIR": "(a) NIR", "LWIR": "(b) LWIR"}
    gamma_ticks_by_regime = {
        "NIR": (10.0, 3.0, 1.0, 0.3),
        "LWIR": (1.0, 0.3, 0.1),
    }
    material_order = {"CdTe": 0}
...
def plot_material_figure_4_style(
    ...
    save_or_show(
        fig,
        save_dir,
        "CdTe_fig4_NIR_scaling.png",
    )


# CODEX MODIFICATION START: CdTe-only plotting workflow
def plot_cdte_figures(
    cases: Sequence[CaseDict],
    n_intensity_points: int = 70,
    save_dir: Optional[Path] = None,
) -> None:
    """Generate all Figs. 1-4 for CdTe."""

    print("\n============================================================")
    print("Generating CdTe plots")
...
# CODEX MODIFICATION END: CdTe-only plotting workflow
...
def plot_total_ionization_3d_surface_grid(
    ...
    """Plot all CdTe total-ionization 3D surfaces in one figure."""
    ...
    fig.suptitle(
        rf"Total ionization surfaces: CdTe {enabled_regions}",
        fontsize=15,
    )
    save_or_show(
        fig=fig,
        save_dir=save_dir,
        filename=f"05_total_ionization_3d_CdTe_{'_'.join(str(case['region']) for case in cases)}.png",
...
def _case_export_metadata(case: CaseDict) -> Dict[str, Any]:
    ...
    metadata: Dict[str, Any] = {
        ...
        "reduced_mass_definition": str(
            CDTE_INPUT["material"]["reduced_mass"]["definition"]
        ),
        "kappa_recorded_not_used": (
            float(CDTE_INPUT["material"]["optical_index"]["kappa"])
            if np.isfinite(float(CDTE_INPUT["material"]["optical_index"]["kappa"]))
            else None
        ),
        "transmission_factor": float(trans),
        "reference_file": str(CDTE_INPUT["reference_file"]),
        "diagnostic_only": bool(
            CDTE_INPUT["material"]["model_status"]["diagnostic_only"]
        ),
        "parameter_status": str(CDTE_INPUT["material"]["parameter_status"]),
    }
    return metadata
...
def save_cdte_plot_variables(
    ...
    """Save long-format CdTe plot data as CSV tables only."""
    ...
    print("\nSaving labeled CdTe plot variables ...")
    for case in cases:
        short = str(case["short"])
        ...
        component_manifest[short] = {
            "used_by": ["CdTe_fig2_NIR.png", "CdTe_fig4_NIR_scaling.png"],
            "arrays": {},
        }
        ...
        component_gamma = np.asarray(
            gamma_cdte_reference_from_intensity_wcm2(
                component["I_wcm2"],
                wavelength_um=float(case["wavelength_um"]),
            ),
            dtype=float,
        )
        component_gamma_name = f"component__{short}__gamma_cdte_reference"
        arrays[component_gamma_name] = component_gamma
        component_manifest[short]["arrays"]["gamma_cdte_reference"] = component_gamma_name
            ...
            component_rows.append(
                {
                    **metadata,
                    ...
                    "gamma_cdte_reference": float(component_gamma[index]),
        ...
        total_gamma_name = f"total_comparison__{short}__gamma_cdte_reference"
        arrays[total_gamma_name] = total_gamma
        total_manifest[short]["arrays"]["gamma_cdte_reference"] = total_gamma_name
            ...
            total_rows.append(
                {
                    **metadata,
                    ...
                    "point_index": int(index),
                    "I_wcm2": float(intensity),
                    "gamma_cdte_reference": float(total_gamma[index]),
    ...
    manifest = {
        "description": "Labeled variables for CdTe Keldysh and avalanche-ionization plots.",
        ...
        "known_limitations": list(
            CDTE_INPUT["material"]["known_limitations"]
        ),
        "units": {
            "I_wcm2": "W/cm^2",
            "F0_jcm2": "J/cm^2",
            ...
            "ne_grid_cm3": "cm^-3",
            "gamma_cdte_reference": "dimensionless",
        },
        "files": {
            "component_scaling_csv": "cdte_component_scaling_long.csv",
            "total_comparison_csv": "cdte_total_comparison_long.csv",
            "surface3d_csv": "cdte_surface3d_long.csv",
        },
        "component_scaling": component_manifest,
        "total_comparison": total_manifest,
        "surface3d": surface_manifest,
    }
    _write_csv(output_dir / "cdte_component_scaling_long.csv", component_rows)
    _write_csv(output_dir / "cdte_total_comparison_long.csv", total_rows)
    _write_csv(output_dir / "cdte_surface3d_long.csv", surface_rows)
    print(f"Saved CdTe CSV data to {output_dir}")
...
def solve_and_display_summary(cases: Sequence[CaseDict]) -> None:
    """Solve the CdTe cases and display the summary table."""
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run the reconciled CdTe Keldysh + avalanche model and "
    ...
    parser.add_argument(
        ...
        default="all",
        help=(
            "all: all CdTe plots and table; summary: first/last graph sets "
            "and table; figures: CdTe Figs. 1-4; 3d: total-ionization "
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Save every currently enabled CdTe figure and labeled numerical variables "
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        choices=range(len(CDTE_INPUT["cases"])),
        metavar="INDEX",
        help=(
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    cases = get_cases()
    material_input = CDTE_INPUT["material"]
    print(f"Running full CdTe workflow ({len(cases)} enabled case(s))")
    ...
    for case in cases:
        I0_wcm2 = case_reference_peak_intensity_wcm2(case)
        gamma0 = gamma_cdte_reference_from_intensity_wcm2(
            I0_wcm2,
            wavelength_um=float(case["wavelength_um"]),
        )
    ...
    if args.save:
        ...
        plot_cdte_figures(cases, args.points, save_dir)
    # CODEX MODIFICATION START: CLI support for 3D surface plot
    elif args.mode == "3d":
