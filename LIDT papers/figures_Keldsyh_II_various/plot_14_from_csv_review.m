...
text(ax, gammaOneI * 1.12, 1e30 / 8, '\gamma=1', 'FontSize', 11, ...
...
sgtitle(fig, superTitle, 'FontWeight', 'bold', 'FontSize', 14);
end
...
for k = 1:numel(gammaTicks)
    ...
    text(ax, xNorm, 1.05, sprintf('%g', gammaTicks(k)), ...
        ...
        'VerticalAlignment', 'bottom', 'FontSize', 8, 'Clipping', 'off');
end
text(ax, 0.5, 1.16, 'Keldysh parameter gamma (ZnS reference)', ...
    'Units', 'normalized', 'HorizontalAlignment', 'center', ...
    'VerticalAlignment', 'bottom', 'FontSize', 9, 'Clipping', 'off');
end
